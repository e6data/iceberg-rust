...
impl CatalogBuilder for BigLakeCatalogBuilder {
    ...
    fn load(
        ...
        async move {
            ...
            BigLakeCatalog::new(self.config, self.storage_factory).await
        }
    }
}
...
impl BigLakeCatalog {
    ...
    pub async fn new(
        ...
        let catalog =
            Self::build_rest_catalog(&config, storage_factory.clone(), &token.access_token).await?;
    ...
    async fn build_rest_catalog(
        ...
        let builder = if let Some(storage_factory) = storage_factory {
            builder.with_storage_factory(storage_factory)
        } else {
            builder
        };

        builder.load(&config.name, props).await
    }
    ...
    async fn refresh_token(&self) -> Result<()> {
        ...
        let new_catalog = Self::build_rest_catalog(
            &self.config,
...
impl Catalog for BigLakeCatalog {
    ...
    /// Drop a table and delete the underlying table files.
    async fn purge_table(&self, table: &TableIdent) -> Result<()> {
        self.ensure_valid_token().await?;

        let result = {
            let state = self.state.read().await;
            state.catalog.purge_table(table).await
        };
